...
import type { Locale } from "@nbetray/openclaw-looki/shared";
...
function parseLocale(value: string | undefined): Locale | undefined {
  ...
  if (!VALID_LOCALES.includes(value as Locale)) {
    throw new CliArgsError(`invalid --locale: ${value}`, "invalidFlagLocale");
  }
...
function parseBaseUrl(value: string | undefined): string | undefined {
  ...
  if (!trimmed) {
    throw new CliArgsError("empty --base-url", "invalidFlagBaseUrl");
  }
  try {
    new URL(trimmed);
  } catch {
    throw new CliArgsError(`not a valid URL: ${trimmed}`, "invalidFlagBaseUrl");
  }
  return trimmed;
}
...
function parseApiKey(value: string | undefined): string | undefined {
  ...
  const trimmed = value.trim();
  if (!trimmed) {
    throw new CliArgsError("empty --api-key", "invalidFlagApiKey");
  }
  return trimmed;
}
...
export function parseCliArgs(argv: string[]): CliArgsResult {
  ...
  try {
    ...
    throw new CliArgsError((err as Error).message, "flagParseError");
  }
